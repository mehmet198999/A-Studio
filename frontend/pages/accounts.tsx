  ...
  return (
    <Layout>
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => fileRef.current?.click()} disabled={csvImporting}
            ...
            {csvImporting ? "Importiere..." : "CSV Import"}
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-5 gap-3 mb-5">
        ...
        <Badge label="OAuth2 aktiv" count={oauth2Count} color="text-yellow-400" />
      ...
      <div className="bg-gray-900/60 border border-gray-800 rounded-lg p-4 mb-4">
        <div className="flex items-start gap-6">
          <div className="flex-1">
            <p className="text-xs font-semibold text-gray-300 mb-1">CSV-Format (minimal – nur 2 Spalten):</p>
            <code className="text-xs text-green-400 bg-gray-800 px-2 py-1 rounded block">email,password</code>
            <div className="mt-2 space-y-1 text-xs text-gray-500">
              <p><span className="text-blue-400">Outlook/Hotmail</span> → Provider automatisch erkannt, OAuth2-Token automatisch geholt</p>
              <p><span className="text-red-400">Gmail</span> → Passwort = 16-stelliges App-Passwort (Google Konto → Sicherheit → App-Passwörter)</p>
              <p><span className="text-green-400">Firstmail</span> → Normales Passwort, direkt nutzbar (SMTP Port 465 SSL)</p>
            </div>
          </div>
          <div className="flex-shrink-0 border-l border-gray-800 pl-4">
            <label className="flex items-center gap-2 cursor-pointer mb-1">
              <input type="checkbox" checked={autoOAuth2} onChange={(e) => setAutoOAuth2(e.target.checked)}
                className="w-4 h-4 accent-blue-500" />
              <span className="text-sm text-gray-300 font-medium">Outlook → OAuth2 automatisch</span>
            </label>
            <p className="text-xs text-gray-600">Benötigt OUTLOOK_CLIENT_ID in .env</p>
          </div>
        </div>
      </div>
      ...
      {/* Single add form */}
      {showAddForm && (
        <form onSubmit={addSingle} className="bg-gray-900 border border-gray-700 rounded-lg p-4 mb-4">
          <div className="flex gap-3 items-end flex-wrap">
            ...
            <button type="submit" className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-1.5 rounded text-sm">Hinzufügen</button>
          </div>
      ...
      {/* Bulk OAuth2 box */}
      <div className="bg-blue-900/20 border border-blue-800 rounded-lg p-3 mb-4 flex items-center justify-between gap-4">
        <div>
          <p className="text-sm font-medium text-blue-300">Outlook OAuth2 Batch-Umwandlung</p>
          <p className="text-xs text-gray-400 mt-0.5">
            Alle Outlook-Accounts mit gespeichertem Passwort → OAuth2-Token automatisch holen
          </p>
        </div>
        <button onClick={runBulkOAuth2} disabled={bulkOAuth2Running}
          className="bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white text-sm px-4 py-2 rounded flex-shrink-0">
      ...
      {/* Bulk action bar */}
      {selected.size > 0 && (
        <div className="bg-blue-900/30 border border-blue-800 rounded-lg px-4 py-2 mb-3 flex items-center gap-3">
          <span className="text-sm text-blue-300 font-medium">{selected.size} ausgewählt</span>
          <button onClick={() => bulkToggle(true)} className="text-xs bg-green-700 hover:bg-green-600 text-white px-3 py-1 rounded">Aktivieren</button>
          <button onClick={() => bulkToggle(false)} className="text-xs bg-yellow-700 hover:bg-yellow-600 text-white px-3 py-1 rounded">Deaktivieren</button>
          <button onClick={bulkDelete} className="text-xs bg-red-800 hover:bg-red-700 text-white px-3 py-1 rounded">Löschen</button>
          <button onClick={() => setSelected(new Set())} className="text-xs text-gray-400 hover:text-gray-200 ml-auto">Auswahl aufheben</button>
        </div>
      )}
